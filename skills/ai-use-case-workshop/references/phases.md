...
Prepare a visual current-versus-proposed comparison from the recorded work for the same final review. Save optional `workflowComparisons` with one entry per use case whose alignment is clear: `candidateId` and a `stages` array of `taskIds` and zero-based `proposedStepIndices`. Each linked current task and proposed step must appear exactly once, in recorded order. Empty current or proposed sides denote added or omitted work; both sides cannot be empty. Do not pair by array position or combine separate candidate proposals into one invented workflow. Use the full names in the visible comparison and keep references internal. If the correspondence is uncertain, leave that mapping out or use an empty comparison list; the workbook shows separate sequences. Reuse known answers rather than asking a new technical question. Show only the implementation components already recorded for that use case. Editing the underlying tasks or candidates clears the derived mapping in the new revision, so refresh it before the next final review.
