...
export function createD1SessionStore(db,{now=()=>new Date(),randomBytes=size=>crypto.getRandomValues(new Uint8Array(size))}={}) {
  ...
  function reference(value,write=false) {
    ...
    if (!parsed.success || (write&&!WRITE_KEY.test(parsed.data.key))) throw new SessionError('NOT_FOUND');
    return parsed.data;
  }
  async function authority(key,write=false) {
    const isWrite=typeof key==='string'&&WRITE_KEY.test(key);
    if (!isWrite&&(write||typeof key!=='string'||!READ_KEY.test(key))) throw new SessionError('NOT_FOUND');
    return {column:isWrite?'write_hash':'read_hash',hash:await hashValue(key)};
  }
  async function session(key,write=false) {
    const auth=await authority(key,write);
    const row=await sql(`SELECT * FROM workshop_sessions WHERE ${auth.column}=?`,[auth.hash]);
    if (!row) throw new SessionError('NOT_FOUND');
    return row;
  }
  function decode(json) {
    try { return validateRecord(JSON.parse(json)); } catch { throw new SessionError('UNAVAILABLE'); }
  }
  ...
  async function load(key,revision) {
    ...
    const auth=await authority(key);
    const row=await sql(`SELECT s.state,s.current_revision,r.record_json,r.revision FROM workshop_sessions s
      LEFT JOIN workshop_revisions r ON r.session_id=s.session_id AND r.revision=${revision===undefined?'s.current_revision':'?'}
      WHERE s.${auth.column}=?`,revision===undefined?[auth.hash]:[revision,auth.hash]);
    if (!row) throw new SessionError('NOT_FOUND');
    active(row);
    if (row.record_json===null) throw new SessionError('NOT_FOUND');
    return {record:decode(row.record_json),reference:{key,revision:row.revision},currentRevision:row.current_revision};
  }
  ...
  return {
    async prepare() {
      const key=await fresh('ws1_'),writeHash=await hashValue(key),readHash=await hashValue(await readKeyFor(key)),stamp=time().toISOString();
      const admitted=await sql(`INSERT INTO workshop_sessions(session_id,write_hash,read_hash,created_at,updated_at)
        SELECT ?,?,?,?,? WHERE (SELECT count(*) FROM workshop_sessions)<?`,[writeHash,writeHash,readHash,stamp,stamp,MAX_SESSIONS],'run');
    ...
    load,
    async loadShared(key,revision) { if (!READ_KEY.test(key)) throw new SessionError('NOT_FOUND'); return load(key,revision); },
    async transact(input,operation,reducer) {
      ...
      const previous=await receipt(row,operation,ref);
      ...
      const result=await sql(`UPDATE workshop_sessions SET current_record=?,current_revision=?,updated_at=?,operation_id=?,operation_hash=?,operation_expected=?
        ...
        AND NOT EXISTS(SELECT 1 FROM workshop_operations WHERE session_id=? AND operation_id=?)`,
      [nextJson,next.revision,time().toISOString(),operation.operationId,operation.operationHash,ref.revision,row.session_id,ref.revision,row.session_id,operation.operationId],'run');
      if (!(result.meta.changes>0)) {
        const replayed=await receipt(row,operation,ref);
        if (replayed) return replayed;
        await session(ref.key,true);
    ...
    async createFileTicket(key,revision,kind) {
      ...
      const auth=await authority(key),ticket=await fresh('wf1_'),ticketHash=await hashValue(ticket),stamp=time(),expiresAt=new Date(stamp.getTime()+15*60*1000).toISOString();
      let result;
      try {
        const results=await db.batch([
          db.prepare(`DELETE FROM workshop_file_tickets WHERE expires_at<=? AND session_id IN
            (SELECT session_id FROM workshop_sessions WHERE ${auth.column}=?)`).bind(stamp.getTime(),auth.hash),
          db.prepare(`INSERT INTO workshop_file_tickets(ticket_hash,session_id,revision,kind,expires_at,created_at)
            ...
            WHERE s.${auth.column}=? AND s.state='active' AND r.revision=?
            AND (SELECT count(*) FROM workshop_file_tickets t WHERE t.session_id=s.session_id AND t.expires_at>?)<?`)
            .bind(ticketHash,kind,Date.parse(expiresAt),stamp.toISOString(),auth.hash,revision,stamp.getTime(),MAX_ACTIVE_TICKETS),
        ]);
        result=results[1];
      } catch(error) { throw storageError(error); }
