...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/healthz') return protectedResponse(JSON.stringify({service:'ai-use-case-workshop',version:'0.8.1', enabled:env.WORKSHOP_ENABLED === 'true', configured:accessConfigured(env), storage:env.WORKSHOP_DB?'persistent-d1':'unconfigured',writesEnabled:env.WORKSHOP_WRITES_ENABLED!=='false',retention:'until-explicit-deletion'}), {headers:{'Content-Type':'application/json'}});
