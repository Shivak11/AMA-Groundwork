...
File transport repair keeps the service stateless. Confirmation generates and validates the cumulative PDF but returns only its manifest alongside the canonical record. A new read-only download_workbook_file tool regenerates the specified snapshot and returns a single gzip resource with a compact manifest; no duplicate record, HTML, font or PDF bytes. The widget unpacks it and uses the existing host download capability. A 140,000-character whole-file-result ceiling fails explicitly rather than silently losing the visual. The PDF's text, tags and links remain byte-for-byte intact after decompression. Text-only file attachment depends on the host's file tools and requires actual testing. No new storage, public links, scopes or service is introduced.

Actual v0.5.0 host testing exposed routine JSON file-materialisation approval in ChatGPT and unsolicited start-tool visuals in Claude despite non-visual metadata. Remove normal-tool embedded JSON attachments: the complete record remains in structured output, ordinary JSON text and manual view downloads. Add view.display=true only to the four intended visual tools; false for routine/error results. A newly mounted view receiving false stays empty, and an existing historical view ignores such a notification rather than replacing its snapshot. Preserve ordinary tool errors and schema recovery in model text. File creation permission remains only for an explicit requested file.

The refreshed ChatGPT v0.5.1 journey saved Step 2 but reconstructed a prior approval using a phase-level confirmation field instead of approvalNote, then guessed timestamps and stopped. Expose the record envelope and approval-field names in tool input schemas; preserve strict domain validation and never normalise or fabricate approval history. Invalid-record failures must return exact record repair guidance, requiring reuse of the last successful canonical record. A view must remain empty until a valid intended snapshot arrives, including hosts that mount an errored tool without delivering its result. No new storage or permissions are authorised by this repair.

Test worker additionally owns tests/chat-workbook.test.mjs, tests/mcp.test.mjs and scripts/verify-chat-workbook.mjs to adapt assertions to locally rendered book HTML and the separate read-only file tool. Preserve the existing behavioural guarantees: no record-mutating calls from the widget, no model-context writes, no lost answers or false delivery claims. Test file bytes by decompressing the dedicated resource. Root owns remaining tests and remote runner changes.
