...
function renderGoal(a) {
  return `<div class="goal-diagram" data-book-visual="goal" aria-label="The outcome, its success measure and safeguards">
    ${node('The outcome we want', a.outcome, 'goal-outcome')}
    ${connector}
    <div class="measure-connection">${node('How we will measure it', a.kpi, 'goal-measure')}${node('What we know about the baseline', a.baseline, 'goal-baseline')}</div>
    ${connector}
    ${node('What must not get worse', a.guardrail, 'goal-guardrail')}
...
function renderGaps(record, a) {
  ...
  return `<div class="gap-map" data-book-visual="gap-map" aria-label="Information, its holder, the barrier and a practical change">${routes || answer(null)}</div>${field('The first gap to address', a.firstGap, 'first-gap')}`;
}
...
function renderCandidates(record, a) {
  ...
  return `<div class="candidate-atlas" data-book-visual="candidate-work-map">${(a.candidates ?? []).map(candidate => {
    ...
    return `<section class="candidate-map${long ? ' long-candidate' : ''}"><h3 class="candidate-title answer">${escapeBookText(candidate.title)}</h3>${dispositions[candidate.id] ? `<p class="recorded-choice">Group choice: ${escapeBookText(dispositions[candidate.id])}</p>` : ''}
...
function renderPriorities(record, a) {
  const rows = ['First', 'Later', 'Do not pursue'].map(decision => {
  ...
  return `<table class="priority-map" data-book-visual="priority-comparison"><caption>The group’s priorities</caption><thead><tr><th scope="col">Use case</th><th scope="col">Our reason</th><th scope="col">Evidence still needed</th></tr></thead>${rows}</table>${field('The strongest challenge to our choice', a.challenge, 'challenge-record')}${field('The recurring effort and cost to account for', a.costs, 'cost-record')}`;
}

function renderTest(record, a) {
  return `<div class="test-plan" data-book-visual="test-plan"><section class="test-decision"><h3>${escapeBookText(recorded(a.decision))}</h3>${answer(a.recommendation)}</section>
    ...
    ${connector}
    <div class="test-action"><h3>The next test or evidence-gathering step</h3>${answer(a.test)}</div>
