...
export async function createPersistentWorkshopServer({sessionStore:store,pdfRenderer,assetLoader:file,capabilitiesOverride,baseUrl,writesEnabled=true}={}) {
  ...
  const server=new McpServer({name:'ai-use-case-workshop',version:'0.8.0'},{instructions:`${PARTICIPANT_LANGUAGE_POLICY} ${COMPLETION_POLICY} ${hostGuide} Internal access policy, explain only when relevant or requested: ${retention}`});
