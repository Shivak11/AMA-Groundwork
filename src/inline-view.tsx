    ...
    {!record || !phase ? <section className="cw-empty"><h1>AI use-case workbook</h1><p>{props.connected ? 'No saved workbook record is included in this card. Continue in the conversation.' : 'Waiting for the host to share this workbook snapshot.'}</p></section> : <>
      ...
      <div className={`cw-layout${props.bookHtml ? '' : ' cw-without-book'}`}>
        <main className="cw-main"><section className="cw-phase" aria-labelledby="snapshot-phase-title"><h2 className="cw-sr-only">Saved work for this step</h2><PhaseVisual record={record} phaseId={phaseId} /></section>
          ...
          {props.workspaceUrl && <section className="cw-persistent-access"><button type="button" disabled={!props.connected || waiting} onClick={() => void run(props.onOpenWorkspace ?? props.onRequestFiles)}>Open the latest saved workbook</button><p>Saved without an expiry. The reading link stays available for later feedback and downloads.</p><details><summary>Keep access for after the workshop</summary><p>Anyone with the reading link can view and download this workbook. Keep the private continuation reference below only within your group; it also permits editing and deletion.</p><p className="cw-access-link">{props.workspaceUrl}</p><pre tabIndex={0} aria-label="Private continuation reference">{JSON.stringify(props.continuation,null,2)}</pre></details></section>}
      ...
      <footer className="cw-snapshot-note">Snapshot of revision {record.revision}. {props.latestRevision !== undefined && props.latestRevision > record.revision ? `The latest saved revision is ${props.latestRevision}. ` : ''}Later conversation changes may not appear in this card.</footer>
