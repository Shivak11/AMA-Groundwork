...
const labels: Record<string, string> = {
  ...
  zeroSecond:'What remains if the task takes no time', redesign:'Proposed workflow change', underlyingProblem:'Underlying problem agreed by the group', candidates:'Use-case candidates', title:'Candidate', taskIds:'Related tasks',
...
function RecordValue({ value, record, field }: { value: unknown; record: WorkshopRecord; field?: string }) {
  ...
  if ((field === 'candidateId' || field === 'id') && typeof value === 'string') {
    ...
    return <span className="cw-wording">{name ? `${name} (${value})` : `Name not available (${value})`}</span>;
  }
  if (field === 'taskIds' && Array.isArray(value)) return <ul>{value.map((id, index) => <li key={index}>{record.phases[2].answers.tasks?.find(task => task.id === id)?.work ?? 'Task name not available'} ({String(id)})</li>)}</ul>;
  if (field === 'decision' && value === 'Do not pilot yet') return <span>Do not begin an implementation test yet.</span>;
  if (field === 'decision' && value === 'Test a use case') return <span>A bounded implementation test is proposed.</span>;
  ...
  if (value !== null && typeof value === 'object') return <dl className="cw-full-fields">{Object.entries(value).map(([key, item]) => <div key={key}><dt>{key==='underlyingProblem'&&record.phases[2].status!=='confirmed'?'Underlying problem to review':labels[key] ?? key}</dt><dd><RecordValue value={item} record={record} field={key} /></dd></div>)}</dl>;
...
export function shouldOpenCompletedWorkbook(props: Pick<InlineProps, 'record' | 'phaseId' | 'isHistorical' | 'latestRevision'>) {
  return Boolean(props.record && props.phaseId === 6 && !props.isHistorical
    && !(props.latestRevision !== undefined && props.latestRevision > props.record.revision)
    && props.record.phases.length === 6 && props.record.phases.every(phase => phase.status === 'confirmed'));
}

export function InlineWorkshop(props: InlineProps) {
  const {record, phaseId} = props;
  const completeView = shouldOpenCompletedWorkbook(props);
  const [bookOpen, setBookOpen] = useState(completeView);
  const completionSeen = useRef(completeView);
  ...
  useEffect(() => {
    // An identical completed reply must not reverse a deliberate Close.
    if (completeView && !completionSeen.current) setBookOpen(true);
    completionSeen.current = completeView;
  }, [completeView]);
  ...
  return <div className="inline-workbook" data-phase={phaseId} aria-busy={waiting}>
    ...
    <section id="workshop-book-dialog" className="cw-book-dialog" ref={book} hidden={!bookOpen} tabIndex={-1} aria-labelledby="book-dialog-title" onKeyDown={event => {if(event.key==='Escape')closeBook();}}>
      <div className="cw-book-toolbar"><div><h2 id="book-dialog-title">AMA-Groundwork: AI Use-Case Portfolio</h2><p className="cw-group-identity">{record.group.name} · {approved} of 6 steps approved</p></div><button type="button" onClick={closeBook}>Close workbook</button></div>
      ...
      <div className="cw-book-actions">{props.hasPdf && <button type="button" className="cw-download-primary" disabled={filesDisabled} onClick={() => void run(() => props.onDownload('pdf'))}>Download PDF</button>}{props.workspaceUrl && <button type="button" disabled={filesDisabled} onClick={() => void run(props.onOpenWorkspace ?? props.onRequestFiles)}>Latest workbook</button>}</div>
      {approved === 6 && <section className="cw-final-overview" aria-label="Identified use cases and group recommendation"><dl className="cw-full-fields"><div><dt>Original problem</dt><dd>{record.group.problem}</dd></div><div><dt>Underlying problem agreed by the group</dt><dd>{record.phases[2].answers.underlyingProblem ?? 'The group has not recorded a separate diagnosis.'}</dd></div></dl><CompactVisual record={record} phaseId={6} /></section>}
      <div className="cw-chapters">{record.phases.filter(item => Object.keys(item.answers).length > 0).map(item => <details className="cw-saved-chapter" key={item.id} open={approved !== 6 && item.id === phaseId}><summary>Step {item.id}: {titles[item.id-1]} <span>{statusLabel(item)}</span></summary><RecordValue value={item.answers} record={record} /></details>)}</div>
      ...
      {props.bookHtml && <details className="cw-print-preview"><summary>Preview PDF</summary><iframe title="AMA-Groundwork: AI Use-Case Portfolio" sandbox="" srcDoc={props.bookHtml} /></details>}
      ...
      <details className="cw-backup"><summary>Group details</summary><dl className="cw-full-fields"><div><dt>Problem</dt><dd>{record.group.problem}</dd></div><div><dt>Members</dt><dd>{record.group.members.join(', ')}</dd></div><div><dt>Context</dt><dd>{record.group.context || 'Not recorded'}</dd></div><div><dt>Date</dt><dd>{record.group.date}</dd></div></dl><details><summary>Saved data</summary><p className="cw-snapshot-note">Saved version {record.revision}. This view does not include later changes.</p><pre tabIndex={0} aria-label="Complete JSON record">{JSON.stringify(record,null,2)}</pre></details></details>
    </section>
  </div>;
}
