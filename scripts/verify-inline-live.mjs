...
const errors=[],checks=[],calls=[];page.on('pageerror',error=>errors.push(error.message));
let result,rejectSync=false,hold=false,release;
...
let frame;
async function mount(next){result=next;rejectSync=false;hold=false;release=null;
...
await page.exposeFunction('sync',async()=>{if(rejectSync)throw new Error('Deliberate context-sync failure');});
let failure;
try{
  ...
  const conflicting=structuredClone(latest);conflicting.structuredContent.record.phases[0].answers.outcome='Conflicting wording at the same revision';
