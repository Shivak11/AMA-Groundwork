...
try {
  ...
  const pdf=result.content.find(b=>b.type==='resource'&&b.resource.mimeType==='application/pdf');
  const bytes=Buffer.from(pdf.resource.blob,'base64');assert.equal(bytes.subarray(0,5).toString(),'%PDF-');assert(bytes.length>10000);
