...
const out=new URL(`../output/persistent-${endpoint.hostname==='127.0.0.1'?'local-http':'remote'}-v090/`,import.meta.url);
await mkdir(out,{recursive:true});
const results=[],refs=[];let client,transport;
const sourceHead=execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim();
const connect=async visual=>{
  client=new Client({name:'fictional-workbook-release-check',version:'0.9.0'},{capabilities:visual?{extensions:{'io.modelcontextprotocol/ui':{mimeTypes:['text/html;profile=mcp-app']}}}:{}});
  transport=new StreamableHTTPClientTransport(endpoint);
  await client.connect(transport);
  assert.equal(client.getServerVersion().name,'ama-groundwork');
